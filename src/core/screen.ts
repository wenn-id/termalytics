 ...
 */
export function liveScreen(opts: ScreenOptions): { stop: () => void } {
  const altScreen = opts.altScreen ?? true;
  const interval = opts.interval ?? 1000;
  const out = process.stdout;

  if (altScreen) out.write(ENTER_ALT_SCREEN);
  ...
  const cleanup = () => {
    ...
    if (altScreen) out.write(EXIT_ALT_SCREEN);
  };
