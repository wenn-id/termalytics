...
function parseArgs(argv: string[]): ParsedArgs {
  const args: ParsedArgs = { command: "", live: false };
  for (const a of argv) {
    if (a.startsWith("--width=")) args.width = parseInt(a.slice(8));
    else if (a.startsWith("--height=")) args.height = parseInt(a.slice(9));
    else if (a.startsWith("--title=")) args.title = a.slice(8);
    else if (a.startsWith("--max=")) args.max = parseFloat(a.slice(6));
  ...
  if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
    try {
      ...
      if (Array.isArray(j)) {
        ...
        if (j.length > 0 && Array.isArray(j[0])) return { series: j as number[][] };
        return { series: [j as number[]] };
      }
      if (j.data) return { series: [j.data] };
      if (j.series) return { series: j.series };
      if (j.value !== undefined) return { series: [], single: j.value, labels: j.label ? [j.label] : undefined };
      if (j.headers && j.rows) return { series: [], headers: j.headers, rows: j.rows };
      if (j.matrix) return { series: [], matrix: j.matrix, labels: j.rowLabels, headers: j.colLabels };
...
async function main() {
  ...
  const render = async () => {
    ...
    switch (args.command) {
      ...
      case "bar":
        ...
        break;
      case "spark":
        if (parsed.series[0]) out = sparkline(parsed.series[0], { width: w });
        break;
      case "gauge":
  ...
  if (args.live) {
    ...
    rl.on("line", (line: string) => {
      ...
      let out = "";
      switch (args.command) {
        case "line": out = lineChart(parsed.series, { width: w, height: h, title: args.title, max: args.max }); break;
